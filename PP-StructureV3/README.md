# PP-StructureV3 High-Performance Service Deployment

[简体中文](README_cn.md)

This directory provides a high-performance service deployment solution for PP-StructureV3 with concurrent request processing support.
...
| Triton Server   | Layout parsing models and pipeline orchestration; model management, dynamic batching, inference scheduling |
...
cd paddleocr-deploy-hps/PP-StructureV3
```
...
|---------|-------------|------|
| `paddleocr-structure-v3` | Triton inference server | 8000 (internal) |
| `paddleocr-structure-v3-gateway` | FastAPI gateway (external entry point) | 8081 |
...
| `GATEWAY_PORT` | 8081 | Host port mapped to gateway |

### Pipeline Configuration

To adjust pipeline configurations (such as model path, batch size, deployment device, etc.), please refer to the Pipeline Configuration section in the [PP-StructureV3 Usage Tutorial](https://github.com/PaddlePaddle/PaddleOCR/blob/main/docs/version3.x/pipeline_usage/PP-StructureV3.en.md).
...
| `useSealRecognition` | boolean | No | false | Enable seal text extraction |
| `useTableRecognition` | boolean | No | true | Convert tables to HTML/Markdown |
| `useFormulaRecognition` | boolean | No | true | Convert formulas to LaTeX |
| `useChartRecognition` | boolean | No | false | Parse charts to tables |
| `useRegionDetection` | boolean | No | true | Enhanced layout detection |
| `visualize` | boolean | No | null | Return visualization images |
...
For the full API specification, please refer to the [Official API Documentation](https://ai.baidu.com/ai-doc/AISTUDIO/Fmfz6oh2e).
...
```bash
docker compose logs paddleocr-structure-v3
docker compose logs paddleocr-structure-v3-gateway
```
