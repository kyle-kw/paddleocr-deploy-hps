# PP-StructureV3 高性能服务化部署

[English](README.md)

本目录提供一套支持并发请求处理的 PP-StructureV3 高性能服务化部署方案。
...
| Triton 服务器  | 版面解析模型及产线串联逻辑，负责模型管理、动态批处理、推理调度 |
...
| 服务 | 说明 | 端口 |
|------|------|------|
| `paddleocr-structure-v3` | Triton 推理服务器 | 8000（内部） |
| `paddleocr-structure-v3-gateway` | FastAPI 网关（对外入口） | 8081 |
...
| `GATEWAY_PORT` | 8081 | 网关映射到宿主机的端口 |

### 产线配置调整

如需调整产线相关配置（如模型路径、批处理大小、部署设备等），请参考 [PP-StructureV3 使用教程](https://github.com/PaddlePaddle/PaddleOCR/blob/main/docs/version3.x/pipeline_usage/PP-StructureV3.md) 中的产线配置调整说明章节。
...
| `useSealRecognition` | boolean | 否 | false | 启用印章文字提取 |
| `useTableRecognition` | boolean | 否 | true | 将表格转换为 HTML/Markdown |
| `useFormulaRecognition` | boolean | 否 | true | 将公式转换为 LaTeX |
| `useChartRecognition` | boolean | 否 | false | 将图表解析为表格 |
| `useRegionDetection` | boolean | 否 | true | 增强版面检测 |
| `visualize` | boolean | 否 | null | 是否返回可视化图片 |
...
完整接口规范请参考 [官方接口文档](https://ai.baidu.com/ai-doc/AISTUDIO/Fmfz6oh2e)。
