...
TRITON_URL = os.getenv("HPS_TRITON_URL", "paddleocr-ocrv5:8001")
...
async def _lifespan(app: fastapi.FastAPI):
    logger.info("Initializing OCR gateway...")
    ...
    logger.info("OCR gateway initialized successfully")
    yield

    logger.info("Shutting down OCR gateway...")
    await app.state.triton_client.close()
    logger.info("OCR gateway shutdown complete")


app = fastapi.FastAPI(
    title="PP-OCRv5 HPS Gateway",
    description="High Performance Server Gateway for PP-OCRv5",
    version="1.0.0",
    lifespan=_lifespan,
)
...
@app.post("/ocr", operation_id="infer", response_class=JSONResponse)
async def handle_ocr(request: Request, body: dict):
    """Handle OCR inference request (PP-OCRv5 API)."""
    request_log_id = body.get("logId", generate_log_id())
    logger.info("Processing OCR request %s", request_log_id)
    ...
    try:
        ...
        logger.warning("Timeout processing OCR request %s", request_log_id)
        return JSONResponse(
            status_code=504,
            content=_create_aistudio_output_without_result(
        ...
        if "Deadline Exceeded" in str(e):
            logger.warning("Triton timeout for OCR request %s", request_log_id)
            return JSONResponse(
                status_code=504,
                content=_create_aistudio_output_without_result(
                    504, "Gateway timeout", log_id=request_log_id
        ...
        logger.error("Triton error for OCR request %s: %s", request_log_id, e)
        return JSONResponse(
            status_code=500,
            content=_create_aistudio_output_without_result(
        ...
        logger.exception("Unexpected error for OCR request %s", request_log_id)
        return JSONResponse(
            status_code=500,
            content=_create_aistudio_output_without_result(
                500, "Internal server error", log_id=request_log_id
    ...
    if output.get("errorCode", 0) != 0:
        ...
        logger.warning("Triton returned error for OCR request %s: %s", request_log_id, error_msg)
        return JSONResponse(
    ...
    logger.info("Completed OCR request %s", request_log_id)
